...
/*
    ...
    Retorna:
        - true, se o PRIMEIRO valor foi MAIOR que o segundo
...
function quickSort(vetor, fnComp, inicio = 0, fim = vetor.length - 1) {
    ...
    if(fim > inicio) {  // Garante que haja, PELO MENOS, DOIS elementos para ordenar
        ...
        quickSort(vetor, fnComp, inicio, posDiv - 1) // Lado esquerdo (números menores que o pivot)
        quickSort(vetor, fnComp, posDiv + 1, fim) // Lado direito (números maiores que o pivot)
    }
}
...
quickSort(candidatos, (a, b) => {
    if(a.NR_CANDIDATO == b.NR_CANDIDATO) {
        if(a.NR_CANDIDATO > b.NM_CANDIDATO) return true
        else return false
    }
