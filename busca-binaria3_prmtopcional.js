// Aula 10/09/2020
// Pré-requisito para a busca binária: o conjunto de dados PRECISA estar ordenado no critério de busca

let comp = 0

// Implementação Recursiva
// Uma função chama a si mesma com pelo menos um de seus parâmetros com valor alterado
// Toda função recursiva precisa de pelo menos uma condição de saída, isto é, uma possibilidade de término que não envolve uma chamada a ela mesma.

// Os parâmetros início e fim são OPCIONAIS. Caso a função seja chamada sem especificá-los, eles assumirão os valores indicados.
function buscaBinaria(lista, valorBusca, fnComp, inicio = 0, fim = lista.length) {

    if(fim >= inicio) {
        // Math.floor(): retira as casas decimais de um número
        let meio = Math.floor((fim + inicio) / 2)

        let resposta = fnComp(lista[meio], valorBusca)
